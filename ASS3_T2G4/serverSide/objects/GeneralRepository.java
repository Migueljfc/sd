   ...
   */
    ...
    */
    public void reportStatus() {
        ...
        switch(chefState){ 
			case States.WAIT_FOR_AN_ORDER:  lineStatus += "\tWAFOR";
		                                    break;
			case States.PREPARING_A_COURSE: lineStatus += "\tPRPCS";
		                                    break;
			case States.DISHING_THE_PORTIONS:      lineStatus += "\tDSHPT";
		                                    break;
			case States.DELIVERING_THE_PORTIONS:    lineStatus += "\tDLVPT";
		                                    break;
			case States.CLOSING_SERVICE:    lineStatus += "\tCLSSV";
                                			break;
        }
        switch (waiterState){ 
         	case States.APPRAISING_SITUATION:  lineStatus += "\tAPPST";
                                                break;
         	case States.PRESENTING_THE_MENU: lineStatus += "\tPRSMN";  
                                                break;
         	case States.TAKING_THE_ORDER:      lineStatus += "\tTKODR";
                                                break;
         	case States.PLACING_THE_ORDER:    lineStatus += "\tPCODR";
                                                break;
         	case States.WAITING_FOR_AN_PORTION:    lineStatus += "\tWTFPT";
                                                break;
         	case States.PROCESSING_THE_BILL:    lineStatus += "\tPRCBL";
                                                break;                                              
         	case States.RECEIVING_PAYMENT:    lineStatus += "\tRECPM";
                                                break;                                            
        }
        for (int i = 0; i < SimulPar.N; i++)
        	switch (studentStates[i]){ 
             	case States.GOING_TO_THE_RESTAURANT:   lineStatus += "\tGGTRT";
                                             break;
             	case States.TAKING_A_SEAT_AT_THE_TABLE: lineStatus += "\tTKSTT";
                                               break;    
             	case States.SELECTING_THE_COURSES: lineStatus += "\tSELCS";
                                               break; 
             	case States.ORGANIZING_THE_ORDER: lineStatus += "\tOGODR";
                                               break; 
             	case States.CHATING_WITH_COMPANIONS: lineStatus += "\tCHTWC";
                                               break; 
             	case States.PAYING_THE_BILL: lineStatus += "\tPYTML";
                                               break; 
             	case States.ENJOYING_THE_MEAL: lineStatus += "\tEJTML";
                                               break; 
             	case States.GOING_HOME: lineStatus += "\tGGHOM";
                                               break; 
